...
export default function UniverseManager() {
  ...
  const [originalSymbols, setOriginalSymbols] = useState<Set<string>>(
    new Set(),
  ...
  const loadFromIndex = useCallback(async (index: string) => {
    ...
    try {
      const res = await fetch(
        `/api/stocks/index?index=${encodeURIComponent(index)}`,
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveSuccess(
        `"${listName.trim()}" saved with ${selectedSymbols.size} stocks!`,
      );
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 p-4 rounded-xl bg-warning/5 border border-warning/20 animate-in fade-in slide-in-from-top-2 duration-300">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-muted mb-2">
                    You&apos;ve changed the selection. Give it a name to save
                    for backtesting.
                  </p>
                  <TextField
                    value={listName}
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <button
                          type="button"
                          onClick={() =>
                            handleDropdownChange(`custom:${wl.id}`)
                          }
