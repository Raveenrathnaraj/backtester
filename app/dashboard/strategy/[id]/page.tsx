      ...
      if (assistantMsg.generatedStrategy) {
        ...
        try {
          ...
          const saveRes = await fetch(
            currentStrategyId
              ? `/api/strategy/${currentStrategyId}`
              : "/api/strategy",
            {
            ...
            if (!currentStrategyId) {
              ...
              if (savedData.id) {
                ...
                window.history.replaceState(
                  null,
                  "",
                  `/dashboard/strategy/${savedData.id}`,
                );
              }
            }
          } else {
                      ...
                      <Modal.Dialog>
                        ...
                        <Modal.Body>
                          <p className="text-sm text-muted">
                            ...
                            This won't delete the strategy itself — it just
                            clears your conversation while keeping the final
                            strategy shown.
                          </p>
                        </Modal.Body>
                        <Modal.Footer>
                  ...
                  <Modal.Backdrop>
                    <Modal.Container>
                      <Modal.Dialog>
                        ...
                        <Modal.Body>
                          <p className="text-sm text-muted">
                            Are you sure you want to delete this strategy? This
            ...
            <AssistantBubble>
              <div>
                ...
                <p className="text-muted text-sm leading-relaxed mb-3">
                  Describe what you have in mind and I&apos;ll ask the right
                  questions to turn it into working code.
                </p>
...
function GeneratedStrategyCard({ strategy }: { strategy: GeneratedStrategy }) {
  ...
  const otherLines = lines.filter(
    (l) => !buyLines.includes(l) && !sellLines.includes(l),
  );
        ...
        {expanded && (
          <div className="rounded-xl border border-white/10 bg-black/85 overflow-hidden">
            <div className="flex items-center gap-1.5 px-3 py-2 border-b border-white/10">
              ...
              <span className="ml-1 text-[10px] text-white/30 font-mono">
                strategy.js
              </span>
            </div>
...
function RuleGroup({
  ...
  return (
    <div
      className={`rounded-lg border ${colorMap.bg} ${colorMap.border} px-3 py-2.5`}
    >
      <div className="flex items-center gap-2 mb-2">
        <span className={`text-xs font-bold ${colorMap.text}`}>{icon}</span>
        <span
          className={`text-xs font-semibold uppercase tracking-wider ${colorMap.text}`}
        >
          {label}
        </span>
      </div>
      <div className="space-y-1">
        {lines.map((line, i) => (
          <p key={i} className="text-xs text-muted flex gap-2 items-start">
            <span
              className={`mt-1.5 w-1 h-1 rounded-full flex-shrink-0 ${colorMap.dot}`}
            />
            <span>
              {line
                .replace(/^[•→↗↘◎\-\*]\s*/u, "")
